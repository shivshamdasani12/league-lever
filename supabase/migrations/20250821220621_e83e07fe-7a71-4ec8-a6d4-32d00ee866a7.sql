...
DROP VIEW IF EXISTS league_matchups_v;
DROP VIEW IF EXISTS league_weeks_v;

-- Create the league_weeks_v view that shows all 18 weeks for all leagues
CREATE VIEW league_weeks_v AS
SELECT 
  l.id::text as league_id,
  w.week,
  CASE 
    WHEN w.week = 1 THEN true 
    ELSE false 
  END as is_latest
FROM public.leagues l
CROSS JOIN (SELECT generate_series(1, 18) as week) w;

-- Create the league_matchups_v view for matchup pairs from sleeper_matchups
CREATE VIEW league_matchups_v AS
SELECT 
  sm1.league_id,
  sm1.week,
  sm1.matchup_id,
