...
DO $$
BEGIN
    -- Only proceed if the projections table exists
    IF EXISTS (
        SELECT 1 FROM information_schema.tables 
        WHERE table_schema = 'public' AND table_name = 'projections'
    ) THEN
...
-- Create additional indexes for better query performance (only if projections table exists)
DO $$
BEGIN
    IF EXISTS (
        SELECT 1 FROM information_schema.tables 
...
-- Add updated_at trigger if it doesn't exist (only if projections table exists)
DO $$
BEGIN
    IF EXISTS (
        SELECT 1 FROM information_schema.tables 
