...
create policy if not exists "Members or owner can view leagues"
  on public.leagues for select
  to authenticated
  using (public.is_league_member(id, auth.uid()) or created_by = auth.uid());

create policy if not exists "Users can create leagues"
  on public.leagues for insert
  to authenticated
  with check (created_by = auth.uid());

create policy if not exists "Owner can update leagues"
  on public.leagues for update
  to authenticated
  using (created_by = auth.uid());

create policy if not exists "Owner can delete leagues"
...
create policy if not exists "Users can view members of their leagues"
  on public.league_members for select
  to authenticated
  using (public.is_league_member(league_id, auth.uid()));

create policy if not exists "Users can join a league"
  on public.league_members for insert
  to authenticated
  with check (user_id = auth.uid());

create policy if not exists "Users can leave their membership"
...
create policy if not exists "League members can view invitations"
  on public.invitations for select
  to authenticated
  using (public.is_league_member(league_id, auth.uid()));

create policy if not exists "League members can create invitations"
  on public.invitations for insert
  to authenticated
  with check (public.is_league_member(league_id, auth.uid()));

create policy if not exists "Invitee can accept invitation"
...
create policy if not exists "League members can view bets"
  on public.bets for select
  to authenticated
  using (public.is_league_member(league_id, auth.uid()));

create policy if not exists "League members can create bets"
  on public.bets for insert
  to authenticated
  with check (created_by = auth.uid() and public.is_league_member(league_id, auth.uid()));

create policy if not exists "Creators or accepters can update their bets"
